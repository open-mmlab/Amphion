...
def main(output_path, dataset_path):
    ...
    speech_lists = {split: sorted(glob(f"{os.path.join(wsj0reverb_path, dic_split[split])}/**/*.wav")) for split in splits}
    ...
    for i_split, split in enumerate(splits):
        ...
        noisy_reverberant_output_dir = os.path.join(save_dir, "audio", split, "noisy_reverb")
        if split == "test":
            unauralized_output_dir = os.path.join(save_dir, "audio", split, "unauralized")
        ...
        for i_sample in tqdm(range(real_nb_samples)):
            if not i_sample % NB_SAMPLES_PER_ROOM:  # Generate new room
                ...
                center_mic_position = np.array(
                    [np.random.uniform(MIN_DISTANCE_TO_WALL, room_dim[n] - MIN_DISTANCE_TO_WALL) for n in
                     range(3)])  # draw source position
                source_position = np.array(
                    [np.random.uniform(MIN_DISTANCE_TO_WALL, room_dim[n] - MIN_DISTANCE_TO_WALL) for n in
                     range(3)])  # draw source position
                mic_array_2d = pra.beamforming.circular_2D_array(center_mic_position[: -1], CHANNELS, phi0=0,
                                                                 radius=MIC_ARRAY_RADIUS)  # Compute microphone array
                mic_array = np.pad(mic_array_2d, ((0, 1), (0, 0)), mode="constant",
            ...
            dry = np.pad(dry, ((0, int(.5 * sample_rate)), (0, 0)), mode="constant",
                         constant_values=0)  # Add 1 second of silence after dry (because very dry) so that the reverb is not cut, and all samples have same length
            ...
            sf.write(os.path.join(reverberant_output_dir, output_filename), 1 / output_scaling * reverberant,
