...
def log_dur_loss(dur_pred_log, dur_target, mask, loss_type="l1"):
    ...
    if loss_type == "l1":
        loss = F.l1_loss(dur_pred_log, dur_target_log, reduction="none").float() * mask.to(dur_target.dtype)
    elif loss_type == "l2":
        loss = F.mse_loss(dur_pred_log, dur_target_log, reduction="none").float() * mask.to(dur_target.dtype)
    else:
...
def log_pitch_loss(pitch_pred_log, pitch_target, mask, loss_type="l1"):
    ...
    if loss_type == "l1":
        loss = F.l1_loss(pitch_pred_log, pitch_target_log, reduction="none").float() * mask.to(pitch_target.dtype)
    elif loss_type == "l2":
        loss = F.mse_loss(pitch_pred_log, pitch_target_log, reduction="none").float() * mask.to(pitch_target.dtype)
    else:
        raise NotImplementedError()
...
def diff_loss(pred, target, mask, loss_type="l1"):
    ...
    # mask: (B, T)
    if loss_type == "l1":
        loss = F.l1_loss(pred, target, reduction="none").float() * (mask.to(pred.dtype).unsqueeze(1))
    elif loss_type == "l2":
        loss = F.mse_loss(pred, target, reduction="none").float() * (mask.to(pred.dtype).unsqueeze(1))
    else:
        raise NotImplementedError()
...
def diff_ce_loss(pred_dist, gt_indices, mask):
    ...
    loss = F.cross_entropy(pred_dist, gt_indices, reduction="none").float()   # (B, nq, T)
