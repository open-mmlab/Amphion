...
class Diffusion(nn.Module):
    ...
    def forward(self, x, x_mask, cond, spk_query_emb, offset=1e-5):
        ...
        """
        diffusion_step = torch.rand(x.shape[0], dtype=x.dtype, device=x.device, requires_grad=False)
    ...
    def get_cum_beta(self, time_step):
        return self.beta_min * time_step + 0.5 * (self.beta_max - self.beta_min) * (time_step ** 2)
    ...
    def reverse_diffusion(self, z, x_mask, cond, n_timesteps, spk_query_emb):
        ...
        for i in range(n_timesteps):
            ...
            if self.cfg.ode_solver == "midpoint":
                x_mid = 0.5 * (xt_ + xt)
                dxt = self.cal_dxt(x_mid, x_mask, cond, spk_query_emb, diffusion_step=t+0.5*h, h=h)
                xt = xt - dxt
    ...
    @torch.no_grad()
    def reverse_diffusion_from_t(self, z, x_mask, cond, n_timesteps, spk_query_emb, t_start):
        h = t_start / max(n_timesteps, 1)
        xt = z
        for i in range(n_timesteps):
            t = (t_start - (i+0.5)*h) * torch.ones(z.shape[0], dtype=z.dtype, device=z.device)
            dxt = self.cal_dxt(xt, x_mask, cond, spk_query_emb, diffusion_step=t, h=h)
            xt_ = xt - dxt
            if self.cfg.ode_solver == "midpoint":
                x_mid = 0.5 * (xt_ + xt)
