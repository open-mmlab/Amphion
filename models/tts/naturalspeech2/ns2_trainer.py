...
from models.tts.naturalspeech2.ns2_loss import log_pitch_loss, log_dur_loss, diff_loss, diff_ce_loss
...
class NS2Trainer(TTSTrainer):
    ...
    def _build_dataloader(self):
        if self.cfg.train.use_dynamic_batchsize:
            ...
            batch_sampler = batch_by_size(train_dataset.num_frame_indices, train_dataset.get_num_frames,
            ...
            batch_sampler = batch_by_size(valid_dataset.num_frame_indices, valid_dataset.get_num_frames,
                                        max_tokens=self.cfg.train.max_tokens * self.accelerator.num_processes,
                                        max_sentences=self.cfg.train.max_sentences * self.accelerator.num_processes,
                                        required_batch_size_multiple=self.accelerator.num_processes)
            batches = [x[self.accelerator.local_process_index::self.accelerator.num_processes] for x in batch_sampler if len(x) % self.accelerator.num_processes == 0]
    ...
    def _build_optimizer(self):
        optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, self.model.parameters()), **self.cfg.train.adam)
        return optimizer
    ...
    def _train_step(self, batch):
        ...
        diff_out, prior_out = self.model(code=code, pitch=pitch, duration=duration, phone_id=phone_id, ref_code=ref_code, phone_mask=phone_mask, mask=mask, ref_mask=ref_mask)
        ...
        if self.cfg.train.diff_ce_loss_lambda > 0:
            pred_indices, pred_dist = self.model.module.latent_to_code(diff_out['x0_pred'], nq=code.shape[1])
        ...
        if self.cfg.train.diff_ce_loss_lambda > 0:
            ...
            for i in range(pred_indices_list.shape[0]):
                pred_acc = np.sum((pred_indices_list[i] == gt_indices_list[i]) * mask_list) / np.sum(mask_list)
                train_losses["pred_acc_{}".format(str(i))] = pred_acc
        
        train_losses["batch_size"] = code.shape[0]
        train_losses["max_frame_nums"] = np.max(batch["frame_nums"].detach().cpu().numpy())
