...
def text2semantic(prompt0_speech, prompt0_text, prompt_speech, prompt_text, prompt_language, target_text, target_language, use_prompt_text=True, temp=1.0, top_k=1000, top_p=0.85, infer_mode = "new"):
    ...
    if infer_mode == "new":
        input_fetures_prompt0, attention_mask_prompt0 = extract_features(prompt0_speech, processor)
...
asr_model = AutoModel(model="paraformer-zh", vad_model="fsmn-vad", vad_kwargs={"max_single_segment_time": 60000}, punc_model="ct-punc", device="cuda:0")
...
def infer(speech_path, prompt_text, target_wav_path, target_text, target_language='zh', speech_path_prompt0=None, prompt0_text=None, temperature=0.2, top_k=20, top_p=0.9, concat_prompt=False, infer_mode="new", idx = 0, epoch=0, spk_prompt_type=""):
    if idx != 0:
        ...
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        save_path = os.path.join(save_dir, f"{os.path.splitext(os.path.basename(target_wav_path))[0]}_infer_{infer_mode}_{idx}_epoch_{epoch}_{spk_prompt_type}.wav")
    else:
        save_dir = os.path.join("The Path to Store Generated Speech", f"{infer_mode}/{spk_prompt_type}")
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        save_path = os.path.join(save_dir, f"{os.path.splitext(os.path.basename(target_wav_path))[0]}_infer_{infer_mode}_epoch_{epoch}_{spk_prompt_type}.wav")
...
save_path = infer_small(speech_path=spk_prompt_wav_path, prompt_text = spk_prompt_text, target_text=target_text, speech_path_prompt0 = prompt0_wav_path, prompt0_text = prompt0_text, infer_mode = "new", save_path = save_path)
