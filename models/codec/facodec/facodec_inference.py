...
class FAcodecInference(object):
    ...
    def inference(self, source, output_dir):
        ...
        z, quantized, commitment_loss, codebook_loss, timbre, codes = (
            self.model.quantizer(z, source_audio[None, ...].to(self.device).float(),
                                 n_c=self.cfg.model_params.n_c_codebooks, return_codes=True))

        full_pred_wave = self.model.decoder(z)

        os.makedirs(output_dir, exist_ok=True)
        source_name = source.split("/")[-1].split(".")[0]
        torchaudio.save(f"{output_dir}/reconstructed_{source_name}.wav", full_pred_wave[0].cpu(), self.cfg.preprocess_params.sr)
    ...
    def voice_conversion(self, source, reference, output_dir):
        ...
        z = self.model.encoder(source_audio[None, ...].to(self.device).float())
        z, quantized, commitment_loss, codebook_loss, timbre = (
            self.model.quantizer(z, source_audio[None, ...].to(self.device).float(),
                                 n_c=self.cfg.model_params.n_c_codebooks))

        z_ref = self.model.encoder(reference_audio[None, ...].to(self.device).float())
        z_ref, quantized_ref, commitment_loss_ref, codebook_loss_ref, timbre_ref = (
            self.model.quantizer(z_ref, reference_audio[None, ...].to(self.device).float(),
                                 n_c=self.cfg.model_params.n_c_codebooks))

        z_conv = self.model.quantizer.voice_conversion(quantized[0] + quantized[1], reference_audio[None, ...].to(self.device).float())
        full_pred_wave = self.model.decoder(z_conv)

        os.makedirs(output_dir, exist_ok=True)
        source_name = source.split("/")[-1].split(".")[0]
        reference_name = reference.split("/")[-1].split(".")[0]
        torchaudio.save(f"{output_dir}/converted_{source_name}_to_{reference_name}.wav", full_pred_wave[0].cpu(), self.cfg.preprocess_params.sr)
