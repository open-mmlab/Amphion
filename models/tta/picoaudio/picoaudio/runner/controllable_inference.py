...
def main():
    ...
    if args.num_samples == 1:
        for batch in tqdm(test_dataloader):
            idx, onset, event_info, _, _, onset_str = batch # idx, onset, event_info, audios, caption, onset_str
            with torch.no_grad():
        ...
        for batch in tqdm(test_dataloader):
            _, onset, event_info, _, caption, onset_str = batch # idx, onset, event_info, audios, caption, onset_str
            with torch.no_grad():
                latents = model.inference({"onset":onset.to(device), "event_info":event_info.to(device)}, scheduler, num_steps, guidance, num_samples, disable_progress=True)
                mel = vae.decode_first_stage(latents)
                wave = vae.decode_to_waveform(mel)
