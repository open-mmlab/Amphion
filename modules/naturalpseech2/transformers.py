...
class TransformerFFNLayer(nn.Module):
    def __init__(self, encoder_hidden, conv_filter_size, conv_kernel_size, encoder_dropout):
        ...
        self.ffn_1 = nn.Conv1d(self.encoder_hidden, self.conv_filter_size, self.conv_kernel_size, padding=self.conv_kernel_size//2)
...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, encoder_hidden, encoder_head, conv_filter_size, conv_kernel_size, encoder_dropout, use_cln):
        ...
        self.ffn = TransformerFFNLayer(self.encoder_hidden, self.conv_filter_size, self.conv_kernel_size, self.encoder_dropout)
...
class TransformerEncoder(nn.Module):
    def __init__(self, enc_emb_tokens=None,
                 encoder_layer=None,
                 encoder_hidden=None,
                 encoder_head=None,
        ...
        self.layers.extend([
            TransformerEncoderLayer(self.encoder_hidden, self.encoder_head,
                                    self.conv_filter_size, self.conv_kernel_size,
                                    self.encoder_dropout, self.use_cln)
...
class DurationPredictor(nn.Module):
    def __init__(self, cfg):
        ...
        for idx in range(self.conv_layers):
            ...
            self.conv += [
                nn.Sequential(
                    nn.Conv1d(in_dim, self.filter_size, self.kernel_size, padding=self.kernel_size//2),
                    nn.ReLU(),
            ...
            if idx % self.cross_attn_per_layer == 0:
                self.cattn.append(
                    torch.nn.Sequential(
                        nn.MultiheadAttention(self.filter_size, self.attn_head, batch_first=True, kdim=self.filter_size, vdim=self.filter_size),
                        nn.LayerNorm(self.filter_size),
                        nn.Dropout(0.2)
                    )
                )
    ...
    def forward(self, x, mask, ref_emb, ref_mask):
        ...
        for idx, (conv, act, ln, dropout) in enumerate(self.conv):
            ...
            if idx % self.cross_attn_per_layer == 0:
                ...
                y_, _ = attn(y_, ref_emb.transpose(1, 2), ref_emb.transpose(1, 2), key_padding_mask=input_ref_mask)
